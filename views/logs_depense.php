...
// Récupération des logs de dépenses
$sql = "SELECT ld.*, u.noms AS utilisateur
        FROM log_depenses ld
        JOIN utilisateur u ON ld.utilisateur_id = u.id
        WHERE ld.supprimer = 1
        ORDER BY ld.date_action DESC";
$logs = $connexion->query($sql)->fetchAll(PDO::FETCH_ASSOC);

// suppression d’un log
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $connexion->prepare("UPDATE log_depenses SET supprimer = 0 WHERE id = ?");
    if ($stmt->execute([$id])) {
    ...
    header('Location: logs_depense.php');
    exit;
}
?>
...
<main id="main" class="main">
    <div class="card p-3">
        <h4 class="mb-3">Historique des modifications des dépenses</h4>
                ...
                <tbody>
                    <?php $i = 1; foreach ($logs as $log): ?>
                        <tr>
                            ...
                            <td>
                                <a href="logs_depense.php?id=<?= $log['id'] ?>"
